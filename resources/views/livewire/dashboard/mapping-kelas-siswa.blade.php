<section>
    <div>
        <h1 class="text-2xl font-semibold text-gray-900 mb-2">Buat Kelas</h1>
        ...
        <div class="py-4 space-y-4">
            ...
            <div class="flex justify-between max-lg:flex-wrap">
                <div class="w-2/4 flex space-x-4  max-lg:w-full">
                    <div class="w-10/12">
                    <x-input.text  wire:model.live.debounce.300ms="filters.search" placeholder="kelas..." />
                    </div>
                </div>
                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                            <x-table.cell colspan="9">
                                @unless ($selectAll)
                                <div>
                                    <span>You have selected <strong>{{ $data->count() }}</strong> kelas, do you want to select all <strong>{{ $data->total() }}</strong>?</span>
                        ...
                        <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $values->id }}" wire:target="filters.search,perPage,gotoPage, previousPage, nextPage">
                            ...
                            <x-table.cell>
                                <span class="text-cool-gray-900 font-medium">{{ $values->kelas->title }} -   {{ $values->title }} ({{ $values->kelas->jenjang->title }}) </span>
                            </x-table.cell>
                            <x-table.cell>
                        ...
                        <x-table.row>
                            <x-table.cell colspan="8">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <span class="font-medium py-8 text-cool-gray-400 text-xl">No kelas found...</span>
                                </div>
                            </x-table.cell>
                        </x-table.row>
                        @endforelse
        ...
        <form wire:submit.prevent="deleteSelected">
            <x-modal.confirmation wire:model.defer="showDeleteModal">
                <x-slot name="title">Delete kelas</x-slot>
                <x-slot name="content">
        ...
        <form wire:submit.prevent="save">
                <x-modal.dialog wire:model.defer="showEditModal">
                    <x-slot name="title">Create/Edit kelas</x-slot>

                    <x-slot name="content">
                        <x-input.group :inline="'true'" for="title" label="kelas" :error="$errors->first('request.title')">
                            <x-input.text type="title" wire:model="request.title" id="title" placeholder="nama kelas" />
                        </x-input.group>

                        <div>
                            <x-input.group :inline="'true'" for="sekolah" label="Nama sekolah" :error="$errors->first('request.sekolah_id')">
                                <div wire:ignore>
                                    <x-input.select wire:model.live.debounce.300ms="request.sekolah_id" :placeholder="__('- Pilih Sekolah -')">
                                        <option value="">- Pilih sekolah -</option>
                                        @foreach ($sekolah as $value)
                                        <option value="{{ $value->id }}">{{ $value->nama_sekolah }}</option>
                                        @endforeach
                                    </x-input.select>
                                </div>
                            </x-input.group>
                            <x-input.group :inline="'true'" for="sekolah" label="Nama kelas" :error="$errors->first('request.kelas_id')">
                                <div wire:ignore>
