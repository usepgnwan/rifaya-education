<section>
    <div>
        <h1 class="text-2xl font-semibold text-gray-900 mb-2">Siswa</h1>
        ...
        <div class="py-4 space-y-4">
            ...
            <div class="flex justify-between max-lg:flex-wrap">
                <div class="w-2/4 flex space-x-4  max-lg:w-full">
                    <div class="w-10/12">
                    <x-input.text  wire:model.live.debounce.300ms="filters.search" placeholder="siswa..." />
                    </div>
                </div>
                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                            <x-table.cell colspan="9">
                                @unless ($selectAll)
                                <div>
                                    <span>You have selected <strong>{{ $data->count() }}</strong> siswa, do you want to select all <strong>{{ $data->total() }}</strong>?</span>
                        ...
                        <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $values->id }}" wire:target="filters.search,perPage,gotoPage, previousPage, nextPage">
                            ...
                            <x-table.cell>
                                <span class="text-cool-gray-900 font-medium">{{ $values->nama_siswa }} </span>
                            </x-table.cell>
                            <x-table.cell>
                                <span class="text-cool-gray-900 font-medium">{{ $values->sekolah->nama_sekolah }} </span>
                            </x-table.cell>
                            <x-table.cell>
                                <span class="text-cool-gray-900 font-medium">{{ $values->kode }} </span>
                            </x-table.cell>
                        </x-table.row>
                        @empty
                        <x-table.row>
                            <x-table.cell colspan="8">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <span class="font-medium py-8 text-cool-gray-400 text-xl">No siswa found...</span>
                                </div>
                            </x-table.cell>
                        </x-table.row>
                        @endforelse
        ...
        <form wire:submit.prevent="deleteSelected">
            <x-modal.confirmation wire:model.defer="showDeleteModal">
                <x-slot name="title">Delete siswa</x-slot>
                <x-slot name="content">
        ...
        <form wire:submit.prevent="save">
                <x-modal.dialog wire:model.defer="showEditModal">
                    <x-slot name="title">Create/Edit siswa</x-slot>

                    <x-slot name="content">
                        <x-input.group :inline="'true'" for="title" label="siswa" :error="$errors->first('request.nama_siswa')">
                            <x-input.text type="nama_siswa" wire:model="request.nama_siswa" id="nama_siswa" placeholder="siswa" />
                        </x-input.group>
