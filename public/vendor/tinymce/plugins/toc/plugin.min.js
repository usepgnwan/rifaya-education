...
(function () {
    ...
    var generateTitle = function (tag, title) {
      var openTag = '<' + tag + ' contenteditable="true" class="font-bold">';
      var closeTag = '</' + tag + '>';
    ...
    var generateTocHtml = function (editor) {
      ...
      return '<div class="' + editor.dom.encode(getTocClass(editor)) + ' w-full   py-5 text-left text-gray-800 break-words   rounded" contenteditable="false">' + html + '</div>';
    };
    var generateTocContentHtml = function (editor) {
      ...
      for (var i = 0; i < headers.length; i++) {
        var h = headers[i];
        h.element.id = h.id; // id element
        var nextLevel = headers[i + 1] && headers[i + 1].level; // cek nextlevel
        if (prevLevel === h.level) {
          html += '<li class="btn-exam">';
        } else {
          for (var ii = prevLevel; ii < h.level; ii++) {
            html += '<ul class="mt-2 w-full"><li class="btn-exam">';
          }
        }
        html += '<a href="#' + h.id + '" class="block text-sky-600 dark:text-slate-400" >' + h.title + '</a>';
        html += '</li>';
    ...
    //   var headers = editor.$(myBody).find(selector);
      var tocClass = getTocClass(editor);
      var $tocElm =  editor.$(myBody).find('.' + tocClass);
      console.log($tocElm)
      if ($tocElm.length) {
    ...
    var toggleState = function (editor) {
      return function (api) {
        var toggleDisabledState = function () {
          return api.setEnabled(editor.mode.isReadOnly() || hasHeaders(editor));
        };
        toggleDisabledState();
